...
public class MinHeap {
    ...
    private void downHeapify(int parentIdx){
        ...
        if(leftChildIdx<heap.size() && this.heap.get(leftChildIdx)<this.heap.get(minIdx)){
            minIdx=leftChildIdx;
        }
        if(rightChildIdx<heap.size() && this.heap.get(rightChildIdx)<this.heap.get(minIdx)){
    ...
    public int removeElem(int element){
        ...
        if(elementIdx!=-1){
            ...
            this.heap.set(elementIdx,Integer.MIN_VALUE);//max infinity
